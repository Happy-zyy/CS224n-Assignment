...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    word_index = [tokens[word] for word in sentence]
    sentVector += np.sum(wordVectors[word_index],axis=0) / len(word_index)
...
def chooseBestModel(results):
    ...
    for result in results:
        if result['test'] > bestScore:
            bestScore = result['test']
            bestResult = result
    ### END YOUR CODE
...
def main(args):
    ...
    dimVectors = wordVectors.shape[1]   # 词向量维度
