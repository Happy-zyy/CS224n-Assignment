...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        t = (max_length - len(sentence))
        if t > 0:
            new_sentence = sentence + [zero_vector] * t
            new_label = labels + [zero_label] * t
            masking = [True] * len(sentence) + [False] * t
        else:
            new_sentence, new_label, masking = sentence[:max_length], labels[:max_length], [True] * max_length
        ret.append((new_sentence, new_label, masking))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(shape=[None, self.max_length, self.config.n_features],name = "input_placeholder" , dtype=tf.int32)
        self.labels_placeholder = tf.placeholder(shape=[None, self.max_length], dtype=tf.int32, name="labels_placeholder")
        self.mask_placeholder = tf.placeholder(shape=[None, self.max_length], dtype=tf.bool, name="mask_placeholder")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings = tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings, dtype=tf.float32),self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable("b_2", shape=(self.config.n_classes), dtype=tf.float32, initializer=tf.constant_initializer(0))
