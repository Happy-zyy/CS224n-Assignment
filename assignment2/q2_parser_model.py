...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        #这里的input_placeholder是一个句子种每个单词在字典中下标组成的向量，需要接一个embeding层，才能变成句子向量
        self.input_placeholder = tf.placeholder(tf.int32, shape = (None, self.config.n_features), name="input_placeholder")
        self.output_placeholder = tf.placeholder(tf.float32, shape = (None, self.config.n_classes), name="output_placeholder")
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0):
        ...
        ### YOUR CODE HERE
        feed_dict = {}
        #random_index = random.sample(range(len(inputs_batch)), (1 - dropout) * len(inputs_batch))
        if labels_batch is not None:
            feed_dict = {self.input_placeholder: inputs_batch,
                         self.output_placeholder: labels_batch,
                         self.dropout_placeholder: dropout}
        else:
            feed_dict = {self.input_placeholder: inputs_batch,
                         # self.labels_placeholder:labels_batch,
                         self.dropout_placeholder:dropout} ### END YOUR CODE
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=self.output_placeholder))
        ### END YOUR CODE
        return loss
